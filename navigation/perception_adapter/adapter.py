...
from navigation.contracts import EdgeObservation, EdgePassability, PerceptionFrame, PerceptionOutcome, PerceptionTranslation
...
class PerceptionAdapter:
    ...
    def translate(self, frame: PerceptionFrame) -> PerceptionTranslation:
        ...
        for target in frame.targets:
            ...
            if region in ("LEFT_BRANCH", "LEFT") and not frame.road_features.has_left_branch:
                return PerceptionTranslation(frame.frame_id, PerceptionOutcome.INCONCLUSIVE, reason="左侧目标与道路特征矛盾")
            if region in ("RIGHT_BRANCH", "RIGHT") and not frame.road_features.has_right_branch:
                return PerceptionTranslation(frame.frame_id, PerceptionOutcome.INCONCLUSIVE, reason="右侧目标与道路特征矛盾")
            if region in ("FORWARD_BRANCH", "FORWARD", "CURRENT_LANE") and not frame.road_features.has_forward_branch:
            ...
            coverage = () if culvert_found or target.kind == "WALL" else self._visibility.coverage(isinstance(state.location, AtNode))
            passability = EdgePassability.BLOCKED if target.kind == "OBSTACLE" else (
                EdgePassability.CLEAR if isinstance(state.location, AtNode) else (current.passability if current else EdgePassability.UNKNOWN)
            )
        ...
        # 没有目标也可能构成可靠负证据：路口确认道路无障碍，同时为涵洞照明累计覆盖。
        if isinstance(state.location, AtNode):
            for region, present in (
                ...
                if edge_id not in observations:
                    observations[edge_id] = EdgeObservation(
                        edge_id=edge_id,
                        passability=EdgePassability.CLEAR,
                        no_culvert_coverage=self._visibility.coverage(True),
