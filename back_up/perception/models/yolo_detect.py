...
class DetectionEngine:
    ...
    def inference(self, frame: np.ndarray, debug_timing: bool = False) -> list:
        ...
        preds = outputs[0].buffer  # 真实形状 [1, 14, 8400, 1] (见 _postprocess 注释)
    ...
    def _postprocess(self, preds: np.ndarray, img_w: int, img_h: int) -> list:
        ...
        真实输出 (已在 RDK X5 真机用 debug_yolo_output.py 确认):
            preds = outputs[0].buffer, shape = [1, 14, 8400, 1], dtype float32
            语义: batch=1, 通道=14 (bbox cx,cy,w,h + 10 类置信度), anchor=8400,
                  末维是 PTQ 编译器 padding 出的冗余 1。
        因此线性内存顺序就是 [14, 8400] 的 row-major（通道在前、anchor 在后），
        展平后按 (14, 8400) 重排再转置即得 [8400, 14]。
        """
        n_attr = 4 + _NUM_CLASSES            # 14
        n_anchor = _NUM_ANCHORS              # 8400

        arr = np.asarray(preds)
        if arr.size != n_attr * n_anchor:
            raise ValueError(
                f"YOLO 输出元素数异常: {arr.size} != {n_attr * n_anchor} "
                f"(shape={arr.shape})，类别数可能不是 {_NUM_CLASSES}，"
                f"请用 test/vision/debug_yolo_output.py 复测")

        # 展平 → [14, 8400] → 转置为 [8400, 14]（每行一个 anchor 的 14 个属性）
        data = arr.reshape(n_attr, n_anchor).T

        # 分离 bbox 和 class scores
        bbox_raw = data[:, :4]   # [anchor, 4]  (cx, cy, w, h)
        scores_all = data[:, 4:]  # [anchor, _NUM_CLASSES]

        # YOLOv8 检测头输出的是纯分类 logit（ultralytics Detect.forward 里
        # 最后 `cls.sigmoid()` 转概率，但 hobot PTQ 导出时不融合该激活，
        # outputs[0].buffer 拿到的是 float32 原始 logit，可能为负 / >1）。
        # 因此这里必须先 sigmoid，再取 max / argmax，否则 raw logit 直接
        # 与 conf_threshold(=0.25) 比较会系统性漏检（logit=0 时概率 0.5，
        # 却会因 0 < 0.25 被误过滤）。
        scores_all = 1.0 / (1.0 + np.exp(-scores_all))  # sigmoid -> 概率
