...
class MathematicalIPM:
    ...
    def __init__(
        ...
        # 若直接提供了标定得到的真实像素内参 (fx/fy/cx/cy)，则优先采用。
        # 这绕开了 focal_length_mm/pixel_size_mm 的凑数假设，直接使用张正友
        # 标定结果，修正焦距误差和光心≠图像中心两个误差源。
        # fx/fy 用于盲区推导时只用其中一个，取 fy 与旧逻辑保持一致。
        if fx_px is not None and fy_px is not None:
            f_x = float(fx_px)
            f_y = float(fy_px)
            c_x = float(cx_px) if cx_px is not None else img_w / 2.0
            c_y = float(cy_px) if cy_px is not None else img_h / 2.0
        ...
        # K
        # 未显式提供标定内参时，回退到「f_x=f_y、光心=图像中心」的旧假设。
