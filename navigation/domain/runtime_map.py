...
class RuntimeMap:
    ...
    def __init__(self):
        # 任务进度
        self.visited_nodes: Set[str] = set()          # 已打卡 RFID 节点
        self.blocked_edges: Set[int] = set()          # 障碍封锁的 edge_id
        self.discovered_culverts: Set[int] = set()    # 已发现（未侦查）涵洞 edge_id
        self.recon_culverts: Set[int] = set()         # 已侦查涵洞 edge_id
        self.culvert_targets: Set[int] = set()        # 需侦查的涵洞目标（仿真注入）
        ...
        self.culvert_endpoint: Dict[int, Optional[str]] = {}
    ...
    def set_culvert_endpoint(self, edge_id: int, endpoint: Optional[str]):
        ...
        """
        self.culvert_endpoint[edge_id] = endpoint
    ...
    def get_culvert_endpoint(self, edge_id: int) -> Optional[str]:
        ...
        return self.culvert_endpoint.get(edge_id)

    def all_culverts_reconed(self) -> bool:
        """是否所有目标涵洞都已侦查完成（读 recon，非 discovered）。"""
        if not self.culvert_targets:
            return True  # 无目标时恒满足
        return self.culvert_targets.issubset(self.recon_culverts)
