...
def draw_debug_panel(
    ...
    if raw_image is not None:
        ...
        if semantic_mask is not None and np.any(semantic_mask):
            semantic_small = cv2.resize(semantic_mask, (cell_w, cell_h), interpolation=cv2.INTER_NEAREST)
            semantic_overlay = np.zeros_like(vis_clean)
            semantic_overlay[semantic_small > 0] = (0, 150, 0)
            vis_clean = cv2.addWeighted(vis_clean, 0.8, semantic_overlay, 0.35, 0)
    else:
        ...
        if semantic_mask is not None and np.any(semantic_mask):
            semantic_small = cv2.resize(semantic_mask, (cell_w, cell_h), interpolation=cv2.INTER_NEAREST)
            vis_clean[semantic_small > 0] = (0, 150, 0)
            vis_clean[clean_small == 255] = (0, 255, 0)
    ...
    clean_label = "Semantic mask + edges" if semantic_mask is not None else "Clean Mask"
    cv2.putText(vis_clean, clean_label, (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

    # ---------- 右上: BEV ----------
    vis_bev = cv2.cvtColor(bev_mask, cv2.COLOR_GRAY2BGR) if bev_mask.ndim == 2 else bev_mask.copy()
    vis_bev = cv2.resize(vis_bev, (cell_w, cell_h))
    cv2.putText(vis_bev, "Mathematical BEV", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

    # ---------- 左下: Lane State ----------
    vis_sw = cv2.cvtColor(bev_mask, cv2.COLOR_GRAY2BGR) if bev_mask.ndim == 2 else bev_mask.copy()
    ...
    left_boundary_pts = vis_points.get("left_boundary_pts", [])
    right_boundary_pts = vis_points.get("right_boundary_pts", [])
    ...
    for boundary_pts in (left_boundary_pts, right_boundary_pts):
        if len(boundary_pts) > 1:
            points = np.asarray([(int(x), int(y)) for x, y in boundary_pts], dtype=np.int32)
            cv2.polylines(vis_sw, [points], False, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    if crossroad_detected and crossroad_y is not None:
        crossroad_y = max(0, min(bev_h - 1, int(round(crossroad_y))))
    ...
    pair_profile = lane_state.get("pair_profile") or "none"
    pair_target_mm = float(lane_state.get("pair_target_mm", 0.0) or 0.0)
    pair_distance_mm = float(lane_state.get("pair_distance_mm", 0.0) or 0.0)
    semantic_coverage = float(lane_state.get("semantic_boundary_coverage", 0.0) or 0.0)

    vis_text = np.zeros((cell_h, cell_w, 3), dtype=np.uint8)
    lines = [
        "===== Math IPM Telemetry =====", "",
        f"Pitch: {camera_pitch_deg:.1f} deg | Track: {physical_track_width_mm:.0f} mm",
        f"Pair: {pair_profile} | d={pair_distance_mm:.1f}/{pair_target_mm:.0f} mm",
        f"Semantic boundary coverage: {semantic_coverage:.2%}",
