...
def map_region_to_traversal(robot_state: RobotState, topology: TrackTopology, region: str) -> Optional[str]:
    ...
    if isinstance(robot_state.location, OnCruiseEdge):
        if region in ("CURRENT_LANE", "FORWARD", "FORWARD_BRANCH"):
            return robot_state.location.traversal_id
        if region in ("JUNCTION_FORWARD_BRANCH", "JUNCTION_LEFT_BRANCH", "JUNCTION_RIGHT_BRANCH"):
            candidates = topology.outgoing_cruise_edges(robot_state.location.to_node_id)
            if region == "JUNCTION_LEFT_BRANCH":
                predicate = lambda difference: 45.0 <= difference <= 135.0
            elif region == "JUNCTION_RIGHT_BRANCH":
                predicate = lambda difference: -135.0 <= difference <= -45.0
            else:
                predicate = lambda difference: abs(difference) <= 45.0
            origin = topology.get_node(robot_state.location.to_node_id)
            ranked = []
            for edge in candidates:
                ...
                difference = (road_heading - robot_state.world_pose.yaw_deg + 180.0) % 360.0 - 180.0
