...
class StateOrchestrator:
    ...
    def __init__(self, agent: Any):
        self._agent = agent
        self._control_layer = getattr(agent, "control_layer", None)

    def do_global_planning(self):
        if self._control_layer is not None and getattr(self._agent, "_use_control_layer", False):
            return self._control_layer.tick()
        agent = self._agent
